...
void record()
{
  ...
  if (noteReading > 0) { // as long as there's at least 1
    tone(BUZZER_PIN, noteReading, duration); 
  }
...
/**************************************************************************/
// 1. There's different voltage passed into every button. 
// This is because of the resistor ladder that we have created.
// Each of the 3 buttons has access to 5v..but then since we have 
// a resistor ladder, the voltage will differ 
// from highest (on the left) to the lowest (on the right) 
//
// 2. the analog input from this button is our "mode" this goes into   
// our input pin on the Arduino. When different buttons are pressed 
// there are different serial inputs 
// 
// 3. The initial 5v coming out of the arduino drops as it goes through
// the resistor ladder, and is then outputted to the speaker
//
// 4. The tone function takes in the parameters we feed it and  
// makes the sound from the speaker come out.. Frequency being one of them, 
// and it outputs the signal from the piezo speaker
